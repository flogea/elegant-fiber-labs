...
const N21model = require('../models/N21model');
  ...
  async addData(req, res, next) {
    try {
      ...
      for (var oneObj in formData) {
        ...
        formData[oneObj].mv(path.join(__dirname, `/../Files/N21/${newFileName}`), (err) => {
          if (err) {
      ...
      await N21model.find({ id_lab }).then(async (result) => {
        if (result.length === 0) {
          try {
            const n21 = new N21model({
              id_lab,
              quantTables,
              data: reducedData,
            ...
            console.log('error save n21', error.message);
            res.status(500).json({ message: 'error save n21' });
          }
        } else {
          try {
            N21model.findOneAndUpdate(
              { id_lab },
              {
                quantTables,
                data: reducedData,
              ...
              (err) => {
                if (err) {
                  console.log(err);
                  res.status(500).json({ message: 'error update n21' });
                }
              },
            );
      ...
      console.log(error);
      res.status(500).json({ message: 'error n21' });
    }
  }
}

module.exports = new n21_controller();
