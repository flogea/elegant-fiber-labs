...
const M14model = require('../models/M14model');
const Summary = require('../models/Summary');
const { format } = require('date-fns');

class m14_controller {
  async getFiles(req, res) {
    try {
      const { id, i } = req.params;

      const directoryPath = path.join(__dirname, './../Files/M14');
      ...
      res.status(500).json('error send files m14 ', error);
    }
  }
  ...
  async getData(req, res) {
    try {
      const { id } = req.params;
      const m14Obj = await M14model.find({ id_lab: id })
        .then((result) => {
          return result;
        })
      ...
      const summaryObj = await Summary.find({ id_lab: id })
        ...
        .catch((err) => res.status(500).json(err));
      const result = summaryObj.concat(m14Obj);
      res.status(200).json({ result });
    } catch (error) {
      console.log(error.message);
      res.status(500).json('error save m14 data ', error);
    }
  }
    ...
    try {
      ...
      if (formData !== null) {
        for (var oneObj in formData) {
          ...
          switch (oneObj) {
            case 'file1':
              newFileName = currentId + '_141_code.v';
              formData[oneObj].name = currentId + '_141_code.v';
              break;
            case 'file2':
              newFileName = currentId + '_141_rtl.pdf';
              formData[oneObj].name = currentId + '_141_rtl.pdf';
              break;
            case 'file3':
              newFileName = currentId + '_141_sim.png';
              formData[oneObj].name = currentId + '_141_sim.png';
              break;
            case 'file4':
              newFileName = currentId + '_142_code.v';
              formData[oneObj].name = currentId + '_142_code.v';
              break;
            case 'file5':
              newFileName = currentId + '_142_rtl.pdf';
              formData[oneObj].name = currentId + '_142_rtl.pdf';
              break;
            case 'file6':
              newFileName = currentId + '_142_sim.png';
              formData[oneObj].name = currentId + '_142_sim.png';
              break;
            case 'file7':
              newFileName = currentId + '_143_code.v';
              formData[oneObj].name = currentId + '_143_code.v';
              break;
            case 'file8':
              newFileName = currentId + '_143_rtl.pdf';
              formData[oneObj].name = currentId + '_143_rtl.pdf';
              break;
            case 'file9':
              newFileName = currentId + '_143_sim.png';
              formData[oneObj].name = currentId + '_143_sim.png';
              break;
            case 'file10':
              newFileName = currentId + '_144_code.v';
              formData[oneObj].name = currentId + '_144_code.v';
              break;
            case 'file11':
              newFileName = currentId + '_144_rtl.pdf';
              formData[oneObj].name = currentId + '_144_rtl.pdf';
              break;
            case 'file12':
              newFileName = currentId + '_144_sim.png';
              formData[oneObj].name = currentId + '_144_sim.png';
              break;
            case 'file13':
              newFileName = currentId + '_145_code.v';
              formData[oneObj].name = currentId + '_145_code.v';
              break;
            case 'file14':
              newFileName = currentId + '_145_rtl.pdf';
              formData[oneObj].name = currentId + '_145_rtl.pdf';
              break;
            case 'file15':
              newFileName = currentId + '_145_sim.png';
              formData[oneObj].name = currentId + '_145_sim.png';
              break;
            case 'file16':
              newFileName = currentId + '_146_code.v';
              formData[oneObj].name = currentId + '_146_code.v';
              break;
            case 'file17':
              newFileName = currentId + '_136_rtl.pdf';
              formData[oneObj].name = currentId + '_136_rtl.pdf';
              break;
            case 'file18':
              newFileName = currentId + '_136_sim.png';
              formData[oneObj].name = currentId + '_136_sim.png';
              break;
          }

          formData[oneObj].mv(path.join(__dirname, `/../Files/M14/${newFileName}`), (err) => {
            if (err) {
              console.log('error mv files', err);
              res.status(500).json({ message: 'error mv files in m14' });
            }
          });
        }
      }
      ...
      if (labId) {
        ...
        M14model.findOne({ id_lab: labId })
          .then(async (result) => {
            try {
              res.locals.data = result;
              await M14model.findOneAndUpdate(
                { id_lab: labId },
                {
                  file1:
                    formData === null
                ...
                (err, doc) => {
                  if (err) {
                    console.log(err);
                    res.status(500).json({ message: 'error update m14' });
                  } else {
                    console.log(doc);
                  }
                },
        ...
        try {
          M14model.findOne({ id_lab }).then(async (result) => {
            if (result) {
              ...
              try {
                ...
                console.log(error.message);
                res.status(500).json({ message: 'error m14' });
              }
            }
          });
        } catch (error) {
        ...
        .then(async (result) => {
          if (result.length === 0) {
            ...
            try {
              ...
              const m14Obj = await M14model.find({ id_lab }).exec();
              ...
              // await fs.writeFile(`${id_lab}_${lab_name}.txt`, `${newData}`, (err) => {
              //   if (err) throw err;
              // });
              next();
            } catch (error) {
...
module.exports = new m14_controller();
